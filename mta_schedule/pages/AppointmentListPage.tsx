...
const columns = (a: {
  ...
  // Fecha (replaces idExposeColumn; uses v7 valueGetter signature)
  {
    ...
    headerName: 'Fecha',
    valueGetter: (_value, row) => dayjs(row.begins_at).format('DD/MM/YYYY'),
  ...
  // Hora
  {
    ...
    valueGetter: (_value, row) => dayjs(row.begins_at).format('HH:mm'),
    renderCell: ({ value }) => <>{value}</>,
    sortComparator: (a, b) =>
      dayjs(String(a), 'HH:mm').valueOf() - dayjs(String(b), 'HH:mm').valueOf(),
  },
