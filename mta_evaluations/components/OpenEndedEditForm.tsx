...
type I_FormFields = { content: string; tags: string, section_title: string | null }
...
const OpenEndedEditForm: T_QuestionForm<I_AnswerOpenEndedDetail> = ({
  ...
  const { content, tags: tagsFromData, section_title } = data
  const { handleSubmit, control } = useForm<I_FormFields>({
    defaultValues: { content, tags: Array.isArray(tagsFromData) ? tagsFromData.join(';') : (tagsFromData ?? ''), section_title: section_title ?? null },
  })
  ...
  const onSubmit: SubmitHandler<I_FormFields> = (f) =>
    submitWithTags(
      f,
      (g) => ({ content: g.content, tags: g.tags, section_title: g.section_title }),
