...
function SemaforoDot({ mi, todos, size = 14 }: { mi: number; todos: number; size?: number }) {
  const s = calcSemaforo(mi, todos)
  const sc = semColors[s]
  return <div title={sc.label} style={{ width: size, height: size, borderRadius: '50%', background: sc.fg, boxShadow: `0 0 6px ${sc.fg}88`, display: 'inline-block', verticalAlign: 'middle', cursor: 'help' }} />
}
...
function Badge({ v, dark }: { v: number | string; dark?: boolean }) {
  return (
    <div style={{ background: dark ? C.navy : C.cardBlue, color: C.white, fontWeight: 800, fontSize: 24, borderRadius: 6, padding: '7px 16px', display: 'inline-block', minWidth: 62, textAlign: 'center' }}>
  ...
  return (
    <div style={{ flex: 1, minWidth: 150, padding: '0 10px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'flex-end' }}>
        ...
        {showSemaforo && typeof mi === 'number' && mi > 1 && typeof todos === 'number' && (
          <div style={{ marginBottom: 6 }}><SemaforoDot mi={mi} todos={todos} /></div>
        )}
      </div>
    </div>
  )
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${vw} ${vh}`} style={{ overflow: 'visible' }}>
      {[0, 20, 40, 60, 80, 100].map((v) => {
        ...
        return <line key={v} x1={padL} y1={lineY} x2={vw - padR} y2={lineY} stroke="#eee" strokeWidth={0.8} />
      })}
      {bars.map((e, i) => {
        const x = padL + i * bw + bw * 0.1
        const bWidth = bw * 0.8
        const fill = e.id === miId ? C.barHi : C.barFill
        return (
          <g key={e.id}>
            <rect x={x} y={by(e.p)} width={bWidth} height={barH(e.p)} fill={fill} rx={2} />
            <text x={x + bWidth / 2} y={by(e.p) - 3} fontSize={6} textAnchor="middle" fill={C.tb} fontWeight={600}>{e.p}%</text>
            <text x={x + bWidth / 2} y={vh - padB + 12} fontSize={6} textAnchor="middle" fill={C.tm}>{e.id}</text>
          </g>
        )
      })}
    </svg>
...
/* ═══════════════════════════════════════════
   WRAPPERS
   ═══════════════════════════════════════════ */
function BluePage({ children, style }: { children: React.ReactNode; style?: React.CSSProperties }) {
  return (
    <div style={{ minHeight: '100vh', background: `linear-gradient(135deg,${C.navyDeep} 0%,${C.blue} 100%)`, color: C.white, fontFamily: F, display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '60px 80px', position: 'relative', ...style }}>
      {children}
    </div>
  )
}
function WhitePage({ children }: { children: React.ReactNode }) {
  return <div style={{ minHeight: '100vh', fontFamily: F, background: C.off, display: 'flex', flexDirection: 'column', position: 'relative' }}>{children}</div>
}
function PageHeader() {
  return <div style={{ position: 'absolute', top: 28, right: 40, zIndex: 5 }}><Logo width={80} height={26} /></div>
}
function PageFooter({ light }: { light?: boolean }) {
  return <div style={{ textAlign: 'right', padding: '0 40px 28px' }}><LogoAustral width={120} height={30} /></div>
}
function SectionTitle({ children }: { children: React.ReactNode }) {
  return <h2 style={{ fontSize: 26, fontWeight: 800, color: C.txt, borderBottom: `3px solid ${C.accent}`, display: 'inline-block', paddingBottom: 6, marginBottom: 28, marginTop: 0 }}>{children}</h2>
}
...
function P1() {
  return (
    <BluePage style={{ justifyContent: 'flex-end', padding: 0 }}>
      <div style={{ position: 'absolute', top: 30, left: 40 }}><LogoAustral width={120} height={30} /></div>
      <div style={{ position: 'absolute', top: 0, right: 0, width: '50%', height: '100%', background: 'linear-gradient(135deg,rgba(10,30,100,0.3),rgba(0,80,180,0.15))', clipPath: 'polygon(30% 0,100% 0,100% 100%,0% 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ fontSize: 16, color: 'rgba(255,255,255,0.25)', textAlign: 'center', letterSpacing: 2 }}>[ Foto estudiante ]</div>
      </div>
  ...
  return (
    <BluePage>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 60 }}>
        <div>
          <div style={{ marginBottom: 36 }}><Logo width={160} height={50} variant="white" /></div>
          <h2 style={{ fontSize: 24, fontWeight: 800, marginBottom: 16, borderBottom: '3px solid rgba(255,255,255,0.3)', paddingBottom: 8, display: 'inline-block' }}>Las pruebas</h2>
          <ul style={{ fontSize: 15, lineHeight: 1.9, paddingLeft: 20, margin: 0 }}>
            ...
            <li style={{ marginTop: 8 }}>Los resultados se presentan:
              <div style={{ paddingLeft: 14, marginTop: 4, fontSize: 14, opacity: 0.85 }}>por años y por materias,<br />por sectores de contenidos y competencias en cada materia,<br />por secciones de cada curso y de cada estudiante.</div>
            </li>
          </ul>
        </div>
        <div>
          <h2 style={{ fontSize: 28, fontWeight: 800, marginBottom: 16, borderBottom: '3px solid rgba(255,255,255,0.3)', paddingBottom: 8, display: 'inline-block' }}>Introducción</h2>
          <p style={{ fontSize: 15, lineHeight: 1.8, margin: '0 0 16px' }}>En el siguiente informe se presentan los resultados obtenidos por las escuelas del Proyecto META (Medición y Evaluación para la Transformación de los Aprendizajes) en la primera aplicación de 2022 (julio/agosto), segunda aplicación de 2022 (noviembre), primera aplicación 2023 (abril) y segunda aplicación 2023.</p>
          <p style={{ fontSize: 15, lineHeight: 1.8, margin: '0 0 16px' }}>El informe brinda a los colegios información sobre resultados de aprendizaje que les permitirá analizar procesos de aprendizaje y, en consecuencia, aspectos y dimensiones de la enseñanza.</p>
          <p style={{ fontSize: 15, lineHeight: 1.8, margin: 0 }}>Y, por tanto, desarrollar planes de profundización y ampliación de esos logros, así como proyectos de mejora de las acciones de enseñanza si así correspondiera.</p>
        </div>
      </div>
      <PageFooter light />
    </BluePage>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 40 }}>
          <div>
            ...
            <ul style={{ fontSize: 14, lineHeight: 2, color: C.tb, paddingLeft: 18, margin: 0 }}>
              <li>Numeración y operaciones.</li><li>Medidas.</li><li>Geometría: cuerpos y polígonos.</li>
              <li>Estadística y probabilidad. A partir de 6º grado.</li><li>Álgebra. En 9º y 12º.</li>
          ...
          <div>
            ...
            <div style={{ fontSize: 14, lineHeight: 1.7, color: C.tb }}>
              <p style={{ marginBottom: 12, marginTop: 0 }}><strong>Reconocimiento de conceptos:</strong> Capacidad de identificar conceptos por medio de ejemplos, casos, atributos o definiciones de los mismos o viceversa.</p>
              <p style={{ marginBottom: 12 }}><strong>Resolución de operaciones (algoritmos):</strong> Capacidad de resolver diversas operaciones matemáticas mediante distintos procedimientos canónicos o no convencionales.</p>
              <p style={{ margin: 0 }}><strong>Resolución de problemas:</strong> Capacidad de resolución de situaciones matemáticas nuevas, integrales y situadas en contextos intra-matemáticos y/o de la realidad cotidiana.</p>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <WhitePage>
      ...
      <div style={{ padding: '50px 80px', flex: 1 }}>
        ...
        <p style={{ fontSize: 14, lineHeight: 1.7, color: C.tb, marginBottom: 20, marginTop: 0 }}>Las competencias seleccionadas para el área de Lenguaje son <strong>comprensión lectora y reflexión sobre los hechos de la práctica del lenguaje</strong>, tanto para texto de ficción, texto informativo, texto argumentativo (9º y 12º).</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
          <div style={{ background: C.white, borderRadius: 8, padding: 24, border: `1px solid ${C.bdr}` }}>
            ...
            <p style={{ fontSize: 13, lineHeight: 1.7, color: C.tb, margin: 0 }}>Capacidad para buscar, localizar, seleccionar y extraer una información de un texto, más concretamente de fragmentos aislados de información relevante.</p>
          </div>
          <div style={{ background: C.white, borderRadius: 8, padding: 24, border: `1px solid ${C.bdr}` }}>
            ...
            <p style={{ fontSize: 13, lineHeight: 1.7, color: C.tb, margin: 0 }}>
              Capacidad para relacionar las distintas partes de un texto para lo que se precisa comprender la interacción entre la cohesión local y global del texto.
              {expanded && <span> Realizar deducciones sobre la intención del autor, el significado por el contexto, etc.; comparar y contrastar información integrando dos o más fragmentos de información del texto.</span>}
            </p>
            <button onClick={() => setExpanded(!expanded)} style={{ marginTop: 10, fontSize: 12, fontWeight: 600, color: C.accent, background: 'none', border: 'none', cursor: 'pointer', padding: 0, textDecoration: 'underline' }}>
  ...
  return (
    <WhitePage>
      ...
      <div style={{ padding: '50px 80px', flex: 1, maxWidth: 960 }}>
        ...
        <p style={{ fontSize: 15, lineHeight: 1.8, color: C.tb, marginBottom: 20, marginTop: 0 }}>Se recuerda que respondiendo a los pedidos de algunas instituciones en el sentido de elevar el nivel de exigencia de las pruebas, hemos agregado 5 ejercicios en cada una de ellas que hemos dado en llamar "ejercicios tipo PISA".</p>
        <p style={{ fontSize: 15, lineHeight: 1.8, color: C.tb, marginBottom: 24 }}>Los contenidos y las competencias que evalúan esos ítems corresponden a los de las pruebas "clásicas" de 40 ejercicios de nuestras META.</p>
        <p style={{ fontSize: 15, lineHeight: 1.8, color: C.tb, marginBottom: 14 }}>Como consecuencia de lo anterior, los informes presentan 3 tipos de resultados:</p>
        <div style={{ paddingLeft: 24, fontSize: 15, lineHeight: 2, color: C.tb, marginBottom: 24 }}>
          <div><strong>a.</strong> Los correspondientes a los 40 ejercicios propios de META a fin de sostener la comparabilidad con los resultados de aplicaciones anteriores.</div>
        ...
        <p style={{ fontSize: 15, lineHeight: 1.8, color: C.tb, fontWeight: 500, margin: 0 }}>Las comparaciones entre instituciones se presentan sobre los 40 ejercicios mencionados en el punto "a".</p>
      </div>
      <PageFooter />
    </WhitePage>
  )
...
function P6() {
  return (
    <BluePage style={{ alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
      <div style={{ position: 'absolute', top: 40, left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: 30, alignItems: 'center' }}>
        <LogoAustral width={120} height={30} /><Logo width={80} height={26} variant="white" />
      </div>
...
function P7({ f, D, allBars }: { f: T_Filters; D: T_D; allBars: T_Bar[] }) {
  return (
    <div style={{ minHeight: '100vh', fontFamily: F, display: 'flex', background: C.off }}>
      <Sidebar filters={f} />
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '20px 24px 40px' }}>
          ...
          <div style={{ background: C.white, borderRadius: 8, border: `1px solid ${C.bdr}`, padding: '18px 18px 10px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', marginTop: 4 }}>
              <Leg c={C.barHi} t="Mi colegio" /><Leg c={C.barFill} t="Todos los colegios" />
            </div>
          </div>
        </div>
      </div>
...
function P10({ f, D }: { f: T_Filters; D: T_D }) {
  return (
    <div style={{ minHeight: '100vh', fontFamily: F, display: 'flex', background: C.off }}>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '20px 24px 40px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18 }}>
            ...
            <div style={{ background: C.white, borderRadius: 8, padding: '18px 22px', border: `1px solid ${C.bdr}` }}>
              ...
              <div style={{ background: C.off, borderRadius: 8, padding: '36px 16px', textAlign: 'center', color: C.tm, fontSize: 13, border: `1px dashed ${C.bdr}` }}>Seleccione ID del Alumno</div>
            </div>
          </div>
        </div>
      </div>
...
function P11({ f, D }: { f: T_Filters; D: T_D }) {
  return (
    <div style={{ minHeight: '100vh', fontFamily: F, display: 'flex', background: C.off }}>
      <Sidebar filters={f} showMateria={false} />
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
...
function P12() {
  return (
    <BluePage style={{ alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
      <Logo width={200} height={64} variant="white" />
      <div style={{ marginTop: 40 }}><LogoAustral width={160} height={40} /></div>
    </BluePage>
  )
}
  ...
  return (
    <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 200, background: 'rgba(4,17,72,0.92)', backdropFilter: 'blur(8px)', padding: '10px 20px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
      <button onClick={() => go(Math.max(0, pg - 1))} disabled={pg === 0}
        style={{ background: 'none', border: 'none', color: C.white, fontSize: 20, cursor: 'pointer', opacity: pg === 0 ? 0.3 : 1, padding: '0 10px' }}>‹</button>
      {Array.from({ length: total }).map((_, i) => (
        <div key={i} onClick={() => go(i)} title={PAGE_LABELS[i]} style={{ width: pg === i ? 28 : 8, height: 8, borderRadius: 4, cursor: 'pointer', background: pg === i ? C.accent : 'rgba(255,255,255,0.3)', transition: 'all 0.2s' }} />
      ))}
      <button onClick={() => go(Math.min(total - 1, pg + 1))} disabled={pg === total - 1}
        style={{ background: 'none', border: 'none', color: C.white, fontSize: 20, cursor: 'pointer', opacity: pg === total - 1 ? 0.3 : 1, padding: '0 10px' }}>›</button>
      <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.55)', marginLeft: 12, minWidth: 80 }}>
        {pg + 1}/{total} — {PAGE_LABELS[pg] ?? ''}
      </span>
    </div>
  )
}
...
/* ═══════════════════════════════════════════
   ROOT
   ═══════════════════════════════════════════ */
function InformeMETAPageInner() {
  const params = useParams()
  const escuelaId = params?.reportId ? Number(params.reportId) : null
  ...
  if (!escuelaId) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', fontFamily: F, color: C.tm, fontSize: 15 }}>
        No se especificó una escuela.
      </div>
    )
  }
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', flexDirection: 'column', gap: 20, background: `linear-gradient(135deg,${C.navyDeep} 0%,${C.blue} 100%)` }}>
        <Spinner />
        <div style={{ color: C.white, fontFamily: F, fontSize: 15 }}>Cargando informe...</div>
      </div>
    )
  }
  ...
  if (error || !data) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', fontFamily: F, color: C.red, fontSize: 15 }}>
  ...
  const D: T_D = {
    ...
    lenComp: data.detalle.lenComp   ?? [],
    lenCont: data.detalle.lenCont   ?? [],
    bpMiL:   data.detalle.boxplotMiLenguaje    ?? null,
    bpAllL:  data.detalle.boxplotTodosLenguaje ?? null,
  }
  ...
  const pages = [
    ...
    <P5  key={4} />,
    <P6  key={5} />,
    <P7  key={6}  f={f} D={D} allBars={allBars} />,
    <P10 key={7}  f={f} D={D} />,
    <P11 key={8}  f={f} D={D} />,
    <P12 key={9} />,
  ]
...
export default withAuth(InformeMETAPageInner, {
