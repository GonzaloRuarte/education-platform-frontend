...
export function assertE2ePayloadContract(contract) {
  assert.equal(contract?.status, "v126_seeded_credentials_payload_builders_ready_runtime_not_executed");
  ...
  assert.equal(contract?.frontend_helper, "frontend/scripts/e2e-payload-contract.mjs");
  ...
  for (const student of contract?.seeded_student_actors || []) {
    assert.equal(student.second_exam_support_pin_required, false, `${student.key} must not require a second support PIN`);
