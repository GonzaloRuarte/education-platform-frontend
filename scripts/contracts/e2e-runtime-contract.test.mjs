...
const validContract = {
  status: "browser_only_runner_ready_runtime_not_executed",
  ...
  frontend_runner_owner: "frontend/scripts/runtime/browser-e2e.mjs",
...
for (const bad of [
  ...
  { label: "student self-service disabled", patch: { student_support_self_service_required: false } },
  { label: "missing browser automation", patch: { browser_automation_required: false } },
  { label: "missing drawer journey", patch: { required_journeys: validContract.required_journeys.filter((journey) => journey !== STUDENT_SUPPORT_DRAWER_SELF_SERVICE) } },
]) {
  assert.throws(
