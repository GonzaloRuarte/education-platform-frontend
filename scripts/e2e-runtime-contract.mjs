...
export function assertE2eRuntimeContract(contract) {
  assert.equal(contract?.status, "v128_browser_only_runner_ready_runtime_not_executed");
  ...
  assert.equal(contract?.browser_compatibility_aliases_deleted, true, "browser compatibility aliases must stay deleted");
  ...
  assert.equal(contract?.second_exam_support_pin_required, false, "student support drawer must not require a second PIN");
  assert.equal(contract?.student_support_surface, STUDENT_SELF_SERVICE_SUPPORT_DRAWER);
  assert.equal(contract?.frontend_runner_owner, "frontend/scripts/browser-e2e.mjs");
