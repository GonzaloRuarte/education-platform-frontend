...
export function assertPageObjectSelectorContract(contract) {
  assert.equal(contract?.status, "v165_student_exam_selector_contract_personal_id_only_ready");
  ...
  const forbidden = new Set(contract?.forbidden_selector_patterns || []);
  for (const key of ["support_pin", "accessibility_pin", "student_pin", "teacher_support_unlock", "second_exam_support_pin", "ReporteAurora", "ReporteSlideContent", "reportes-aurora", "rb-setup-workbook-organization-select", "/api/setup-workbook/commit-organizations/"]) {
    assert.ok(forbidden.has(key), `forbidden selector/policy fragment missing: ${key}`);
  }
}
