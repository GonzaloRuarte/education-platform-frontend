...
export const REQUIRED_FRONTEND_BROWSER_SMOKE_JOURNEYS = Object.freeze([
  ...
  STUDENT_SUPPORT_DRAWER_WITHOUT_SECOND_PIN,
]);
...
export const REQUIRED_FRONTEND_BROWSER_SMOKE_NEGATIVE_PROBES = Object.freeze([
  ...
  "stale_or_incomplete_snapshot_is_not_presented_as_current",
  "student_support_drawer_opens_without_pin_and_returns_focus",
  "accessibility_controls_do_not_require_teacher_mediation",
  "download_button_has_deliberate_action_but_no_second_pin",
...
export function assertFrontendBrowserSmokeContract(contract) {
  assert.equal(contract?.script_owner, "frontend/scripts/docker-smoke.mjs", "backend contract should name the frontend smoke script owner");
  ...
  assert.equal(contract?.student_exam_support_controls?.pin_gate_allowed, false, "student support drawer must not require a PIN gate");
  assert.equal(contract?.student_exam_support_controls?.separate_button_pin_deleted, true, "separate support/download/accessibility PIN should be deleted");
  assert.equal(contract?.student_exam_support_controls?.teacher_mediation_required, false, "student support drawer must not require teacher mediation");
  assert.equal(contract?.student_exam_support_controls?.access_pattern, STUDENT_SELF_SERVICE_SUPPORT_DRAWER, "support controls should use the student self-service drawer");
