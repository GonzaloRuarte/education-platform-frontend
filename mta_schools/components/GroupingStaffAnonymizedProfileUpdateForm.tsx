...
import { useGroupingStaffAnonymizedProfileUpdate, useNavigateToGroupingStaffAnonymizedProfileList } from '@/mta_schools/hooks'
...
const GroupingStaffAnonymizedProfileUpdateForm = ({ data }: I_Props) => {
  ...
  const update = useGroupingStaffAnonymizedProfileUpdate()

  const onSubmit: SubmitHandler<I_FormFields> = (payload) => {
    setInProgressStatus(true)
    update(data.id, payload)
      .then(() => {
        successToast(sentence(`${GROUPING_STAFF_ANON_PROFILE_NAME.singular} editado correctamente`))
        navToList()
      })
...
export default GroupingStaffAnonymizedProfileUpdateForm
